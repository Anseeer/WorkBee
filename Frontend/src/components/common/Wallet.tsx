  ...
  return (
    <div className="bg-gray-50 flex flex-col items-center p-2 sm:p-3 md:p-4 min-h-[400px]">
      <main className="w-full max-w-4xl">
        ...
        <div className="rounded-lg flex flex-col items-center">
          <div className="bg-gradient-to-r from-green-700 to-blue-900 text-white rounded-xl p-3 sm:p-4 md:p-5 w-full max-w-[280px] sm:max-w-[320px] md:max-w-[360px] h-[120px] sm:h-[140px] md:h-[150px] flex justify-between items-center shadow-md">
            <div className="flex flex-col gap-2 sm:gap-3">
              <div className="text-xl sm:text-2xl md:text-3xl font-bold">₹{balance?.toFixed(2)}</div>
        ...
        {/* Transactions */}
        <div className="bg-gray-50 rounded-lg p-2 sm:p-3 md:p-4 mt-4 sm:mt-6">
          <div className="text-base sm:text-lg md:text-xl font-bold mb-1 sm:mb-2">Recent Transactions</div>
          {history.length > 0 ? (
            <div className="border rounded flex flex-col h-auto min-h-[200px] sm:h-[230px]">
              {/* Table Wrapper */}
                ...
                <table className="w-full text-left text-xs sm:text-sm md:text-base">
                  ...
                  <tbody>
                    {currentItems.map((tx) => (
                      ...
                      >
                        <td
                          className={`p-1 sm:p-2 font-bold ${tx.type === "CREDIT"
                            ? "text-green-600"
                            : "text-red-600"
                            }`}
                        >
              ...
              {/* Pagination fixed at bottom */}
              <div className="flex justify-between items-center m-1 sm:m-2 bg-white text-xs sm:text-sm">
                <button
                  onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))}
                  disabled={currentPage === 1}
                  className={`px-1 sm:px-2 py-0.5 sm:py-1 rounded ${currentPage === 1
                    ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                    : "bg-green-900 text-white hover:bg-green-600"
                    }`}
                >
                ...
                <button
                  ...
                  disabled={currentPage === totalPages}
                  className={`px-1 sm:px-2 py-0.5 sm:py-1 rounded ${currentPage === totalPages
                    ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                    : "bg-green-900 text-white hover:bg-green-600"
                    }`}
                >
            ...
            <div className="text-gray-500 text-xs sm:text-sm md:text-base text-center py-4 sm:py-6">Empty History</div>
          )}
        </div>
      </main>
    </div>
