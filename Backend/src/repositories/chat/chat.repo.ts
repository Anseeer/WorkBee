...
export class ChatRepository extends BaseRepository<IChat> implements IChatRepositoy {
    ...
    async updateChatOnSendMessage(chatId: string, receiverId: string, messageId: string, receiverViewing: boolean): Promise<void> {
        if (!chatId || !receiverId || !messageId) {
            throw new Error("Cant get chatId or receiverId, messageId")
        }
